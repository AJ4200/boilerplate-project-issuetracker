...
suite("Functional Tests", function () {
  // Variable to store a valid _id
  let validId;

  // Before running tests, create an issue to get a valid _id
  suiteSetup((done) => {
    chai
      .request(server)
  ...
  suite("Test PUT", () => {
    test("Test PUT to update one field", (done) => {
      chai
        ...
        .send({ _id: validId, created_by: "Sam" })
        .end((err, res) => {
          assert.equal(res.status, 200);
    ...
    test("Test PUT to update multiple fields", (done) => {
      chai
        .request(server)
        .put("/api/issues/apitest")
        .send({ _id: validId, created_by: "Sam", issue_text: "Sir" })
        .end((err, res) => {
          assert.equal(res.status, 200);
          assert.equal(res.body.result, "successfully updated");
          assert.equal(res.body._id, validId);
